...
void SearchDescriptionOrName(Task task, std::string descriptionOrname, std::string checkNameOrDescr, int k)
{
    for (int i = 0; i < descriptionOrname.size() + 1; i++)
    {
        if (descriptionOrname.size() + 1 == checkNameOrDescr.size() + 1)
        {
            if (descriptionOrname[i] == checkNameOrDescr[i])
            {
                k++;
            }
        }
    ...
    if (k == descriptionOrname.size() + 1)
    {
        PrintTaskInProcess(task);
    }
}

void TaskWeekOrDay(Task task, char weekORday, int i)
{
    if (task.deadline[i] == weekORday)
    {
        PrintTaskInProcess(task);
    }
}
...
void PrintTasksByWeek(std::vector<Task> & tasks)
{
    ...
    for (Task& task : tasks)
    {
        ...
        {
            int i = 3;
            TaskWeekOrDay(task, week,i);
        }
        else if (task.deadline.size() == 3)
        {
            int i_f = 2;
            TaskWeekOrDay(task, week, i_f);
        }
    }
}
...
void SearchBynameTask(std::vector<Task>& tasks)
{
    ...
    for (Task& task : tasks)
    {
        std::string name_n = task.name;
        SearchDescriptionOrName(task, name, name_n, k);
    }
}
...
void SearchByDescription(std::vector<Task>& tasks)
{
    ...
    for (Task& task : tasks)
    {
        std::string description_n = task.description;
        SearchDescriptionOrName(task, description, description_n, k);
    }
}
